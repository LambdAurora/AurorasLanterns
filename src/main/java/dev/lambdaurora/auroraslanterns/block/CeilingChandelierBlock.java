 ...
 * @since 1.5.0
 */
public class CeilingChandelierBlock extends CandleBlock {
	...
	private static final List<Vec3>[] CANDLES_TO_PARTICLE_OFFSETS = Util.make(() -> {
		...
		return new List[]{
				ImmutableList.of(new Vec3(0.5, 0.6875, 0.5)),
				ImmutableList.of(new Vec3(0.25, highestPoint, 0.5), new Vec3(0.75, second, 0.5)),
				ImmutableList.of(
	...
	public static Properties properties(Block source) {
		return Block.Properties.ofFullCopy(source).lightLevel(state -> Math.clamp(CandleBlock.LIGHT_EMISSION.applyAsInt(state) + 2, 0, 15));
	}
}