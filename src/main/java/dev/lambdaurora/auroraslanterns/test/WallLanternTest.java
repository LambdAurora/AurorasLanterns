...
public class WallLanternTest {
	...
	public void tagTest(GameTestHelper context) {
		...
		LanternRegistry.forEach((id, block) -> {
			...
			context.assertTrue(
					state.is(tag),
					Component.literal("Wall lantern %s is not part of the auroraslanterns:wall_lanterns tag.".formatted(id))
			);
			context.assertTrue(
					state.is(BlockTags.MINEABLE_WITH_PICKAXE),
					Component.literal("Wall lantern %s is not part of the pickaxe mineable tag.".formatted(id))
			);
		});
	...
	public void wallLanternAttachment(GameTestHelper context) {
		final var wallLantern = AurorasLanternsRegistry.WALL_LANTERN_BLOCK.defaultBlockState()
				...
				.setValue(WallLanternBlock.FACING, context.getTestRotation().rotate(Direction.NORTH));
		final var lanternPos = BlockPos.containing(2, 2, 0);

		context.runAtTickTime(1, () -> {
			assertBlockState(context, wallLantern, lanternPos);

			context.pressButton(BlockPos.containing(2, 3, 4));
